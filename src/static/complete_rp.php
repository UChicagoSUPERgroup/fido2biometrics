    ...
    <div class="row">
        <div class="col-12">
            <form id="continue-form">
                <?php
                // Study 3 Part 1
                echo '<a class="btn btn-primary btn-lg btn-block" onclick="logEvent(\'clicked()->Continue->'.$_SESSION['status'].'\');" href="https://example.tech/static/auth.php?d1='.urlencode(Base64Url::encode($_SESSION['prolific'])).'&d2='.urlencode(Base64Url::encode($_SESSION['status'])).'&d3='.urlencode(Base64Url::encode($_SESSION['notification'])).'" role="button">Continue with Survey</a>';
                ?>
            </form>
        </div>
    </div>
